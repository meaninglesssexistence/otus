...
// и отправкой данных на Memcache сервер. Для непосредственно посылки
// данных используется еще одна горутина. Если она не занята, данные
// передаются ей, если занята - сохраняются в очереди.
func sendToMemcache(ch <-chan *AppsInstalled, stat *Stat,
                    memCacheIP string, dry bool, exit chan bool,
                    wg *sync.WaitGroup) {
    defer wg.Done()

    sendChan := make(chan *PackedData)
    ...
    queue := make([]*PackedData, 0)

    for {
        select {
            case <-exit:
                // Отправляем оставшиеся данные и завершаем работу.
                    ...
                    if dry {
                        ...
                        select {
                            case sendChan <- pair:
                                ...
                                // Уберем их из очереди
                                queue = queue[1:]
                            default:
                                // Отправщик данных занят
                                // Не убираем данные из очереди
                        }
                    }
                }
        }
...
func main() {
    ...
    dataChannels := map[string](chan *AppsInstalled){
        "idfa": make(chan *AppsInstalled),
        "gaid": make(chan *AppsInstalled),
        "adid": make(chan *AppsInstalled),
        "dvid": make(chan *AppsInstalled),
    }
