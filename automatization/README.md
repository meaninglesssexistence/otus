...
## Архитектура
Сервер реализован на основе asyncio. Используются потоки (stream), а не
прямой доступ к сокетам. На основе очереди сделано ограничние количества
обработчиков запросов. Метод `_handle_connection` принимает подключение
и помещает его в очередь. А N запущенных обработчиков `_worker` берут
очередной запрос из очереди и обрабатывают его.
